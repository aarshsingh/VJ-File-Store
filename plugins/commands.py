...
# Ask Doubt on telegram @KingVJ01
    ...
    if not files_:
        ...
        try:
            ...
            if AUTO_DELETE_MODE == True:
                k = await client.send_message(chat_id = message.from_user.id, text=f"<b><u>❗️❗️❗️IMPORTANT❗️️❗️❗️</u></b>\n\nThis File/Video will be deleted in <b><u>{AUTO_DELETE} minutes</u> 🫥 <i></b>(Due to Safety purpose)</i>.\n\n<b><i>HOWEVER YOU CAN WATCH IT LATER BY USING ITS LINK AGAIN 🥰</b>")
                await asyncio.sleep(AUTO_DELETE_TIME)
                try:
                    await msg.delete()
                except:
...
# Ask Doubt on telegram @KingVJ01
    ...
    if AUTO_DELETE_MODE == True:
        k = await client.send_message(chat_id = message.from_user.id, text=f"<b><u>❗️❗️❗️IMPORTANT❗️️❗️❗️</u></b>\n\nThis File/Video will be deleted in <b><u>{AUTO_DELETE} minutes</u> 🫥 <i></b>(Due to Safety purpose)</i>.\n\n<b><i>HOWEVER YOU CAN WATCH IT AGAIN USING ITS  LINK 🥰  </b>")
        await asyncio.sleep(AUTO_DELETE_TIME)
        try:
            await x.delete()
        except:
